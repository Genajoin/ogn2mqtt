class APRSParser {
  constructor(config = {}, logger = null) {
    this.config = {
      allowedAircraftTypes: config.allowedAircraftTypes || [1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12], // все типы кроме unknown, parachute, static_obstacle
      trackAircraftTypes: config.trackAircraftTypes || [], // типы для отслеживания в логах
  ...
  parsePositionMessage(header, body) {
    ...
    // Проверяем, нужно ли логировать этот тип
    this.logTrackedAircraft(ognData.aircraftType, ognData.deviceId, header.sourceCall);
  ...
  logTrackedAircraft(aircraftType, deviceId, sourceCall) {
    // Проверяем, есть ли этот тип в списке отслеживаемых
    if (this.config.trackAircraftTypes.includes(aircraftType)) {
      const typeName = this.aircraftTypes[aircraftType] || 'unknown';
      this.log('info', `Обнаружен отслеживаемый тип ЛА: ${typeName} (${aircraftType}) - устройство ${deviceId}`, {
        aircraftType,
